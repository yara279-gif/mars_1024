...
class VSdataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # - NO need to duplicate the horizontally-stacked feature vectors of video[idx] into 3 channels to treat them as a 2D image like in CSTA
        # - Just use the horizontally-stacked feature vectors of video[idx] directly

        # output_feature = output_feature.unsqueeze(0).expand(3,-1,-1)
        # if self.transform is not None:
        #     output_feature=  self.transform(output_feature)
        return torch.unsqueeze(output_feature,0),torch.from_numpy(self.gtscores[idx]).float(),self.dataset_names[idx],self.video_nums[idx]  # torch.unsqueeze(output_feature,0)
    
def collate_fn(sample):
    return sample[0]

# Create Dataloader
def create_dataloader(dataset, data_path): # data_path is now a parameter
    loaders = []
